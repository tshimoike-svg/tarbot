...
- **`execution/order_engine.py` 実装済み**（2026-07-18）：`risk_manager.py`のゲートを
  ...
  ゲート判定のみ行いkabuへは一切送信しない。信用区分は**一般信用（無期限）に統一**
  （制度信用の逆日歩リスク回避のため）。発注方法は**寄成行**固定（指値・取消は未実装・
  スコープ外）。`KabuClient`に発注用「取引パスワード」（APIパスワードとは別）解決と
  `send_order`（POST /sendorder）を追加。kabu APIの各コード値（MarginTradeType等）は
  設計時点の理解による暫定値のため、DRY_RUN=Falseで実際に使う前に公式仕様書で要確認。
