...
- **証券口座**：三菱UFJ eスマート証券（旧auカブコム証券）で口座開設・信用取引・
  ...
  `scripts/kabu_ping.py`での疎通確認も完了（2026-07-18・本番/検証とも成功）。
- **`execution/order_engine.py` 実装済み**（2026-07-18）：`risk_manager.py`のゲートを
  必ず通し、承認された注文のみ`KabuClient.send_order`へ渡す。DRY_RUN=Trueの間は
  ゲート判定のみ行いkabuへは一切送信しない。信用区分は**一般信用（長期＝無期限）**
  に統一（MarginTradeType=2。制度信用の逆日歩リスク回避のため）。発注方法は
  **寄成（前場）**固定（FrontOrderType=13。指値・取消は未実装・スコープ外）。口座区分
  は**一般口座**（AccountType=2、実口座で確認済み）。`KabuClient`に発注用「取引パス
  ワード」（APIパスワードとは別。実際はログインパスワードと同じ値で疎通確認済み）の
  解決、`send_order`（POST /sendorder）、`get_positions`（GET /positions・決済に必須の
  返済建玉ID=ExecutionIDを調べる）を追加。
  **kabu APIの各コード値は公式OpenAPI仕様書（kabucom/kabusapiリポジトリの
  `reference/kabu_STATION_API.yaml`）で確認し、検証環境（--env demo）で新規・決済
  両方のsendorderが`Result: 0`で成功することを実際に確認済み**（2026-07-18）。
  信用返済には`ClosePositions`（建玉ID+数量）が必須で、`OrderEngine.submit()`は
  `is_entry=False`のとき`hold_id`引数を要求する（未指定はValueError）。
