...
最大ドローダウンは2種を提供する：
- `max_drawdown`：「各トレード同一ノートーション」のプロキシ（銘柄間・戦略間の相対比較向け）
- `sized_max_drawdown`：`risk_per_trade`（既定 0.5%）× 損切り幅からサイズを逆算した口座レベルDD
  Phase 0 ゲートは `sized_max_drawdown` を使用（プロキシは過大で不当に厳しいため）。
"""
...
def max_drawdown(returns: Sequence[float]) -> float:
    """各トレード同一ノートーションを仮定した累積リターン曲線の最大ドローダウン（プロキシ）。
...
_DEFAULT_STOP_FRAC = 0.02  # stop_price 未設定時のフォールバック（2% 想定）


def sized_max_drawdown(
    results: Sequence["TradeResult"],
    *,
    risk_per_trade: float = 0.005,
    max_size_frac: float = 0.20,
    fallback_stop_frac: float = _DEFAULT_STOP_FRAC,
) -> float:
    """risk_per_trade ベースのサイズで再計算した口座レベルの最大ドローダウン。

    各トレードの stop_price から損切り幅を求め、position_sizer と同じロジックで
    「トレードが1回損切りに遭うと口座の risk_per_trade 分だけ減る」ようにサイズを設定する。
    stop_price が None のトレードは fallback_stop_frac を使用。
    """
    peak = 0.0
    equity = 0.0
    max_dd = 0.0
    for r in results:
        ...
        stop = r.trade.stop_price
        if stop is not None and entry > 0 and abs(entry - stop) > 0:
            stop_frac = abs(entry - stop) / entry
        else:
            stop_frac = fallback_stop_frac
        size_frac = min(risk_per_trade / stop_frac, max_size_frac)
        account_return = r.net_return * size_frac
        equity += account_return
        peak = max(peak, equity)
        max_dd = max(max_dd, peak - equity)
    return max_dd
...
class EvaluationResult:
    ...
    max_drawdown: float         # プロキシ：同一ノートーション仮定の累積DD（比較向け）
    sized_max_drawdown: float   # 口座レベルDD：risk_per_trade=0.5%のサイジングで計算（ゲート用）
...
def _aggregate(results: Sequence[TradeResult]) -> EvaluationResult:
    ...
    return EvaluationResult(
        ...
        sized_max_drawdown=sized_max_drawdown(results),
...
class GateReport:
    ...
    drawdown_ok: bool              # 最大DD < しきい値（sized_max_drawdown ベース）
    walkforward_stable: bool       # 各区間でも期待値が正
    n_trades: int
    expectancy: float
    max_drawdown: float            # プロキシ（参考値）
    sized_max_drawdown: float      # ゲート判定に使用する口座レベルDD
    min_trades: int
...
def check_phase0_gate(
    ...
    *,
    ...
    # sized_max_drawdown（0.5%リスクサイジング）でゲート判定。プロキシDDは過大で不当に厳しい。
    drawdown_ok = result.sized_max_drawdown < max_drawdown_threshold
    ...
    return GateReport(
        ...
        sized_max_drawdown=result.sized_max_drawdown,
